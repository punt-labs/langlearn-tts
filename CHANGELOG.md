...
- Pin `punt-vox<2.0.0` to unblock installation. Tests and source reach into
  ...
  `DEFAULT_VOICES`, `format_voice_hint`), none of which are in vox's `__all__`.

  Two known breakages in the removed range: vox `4.6.0` renamed
  `_voices_loaded` → `_voices_loaded_at` (breaks test collection), and `4.9.0`
  removed `VOICES` entirely (breaks CLI subcommand startup via `get_provider`,
  though `langlearn-tts --help` still loads). `<4.9.0` would only fix the
  `ImportError`; `<4.6.0` might work but has no CI evidence. `<2.0.0` matches
